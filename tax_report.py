...
def generate_tax_report(data: dict, output_path: str, changes: dict = None, config: dict = None) -> str:
    """
...
def _build_summary_sheet(ws, data, config):
    ...
    headers = [
        'Airport', 'Country', 'Tax Code', 'Tax Name', 'Terminal / Category',
        'Condition / Time Range', 'Currency', 'Amount', 'Status'
    ]
        ...
        for tax_type in airport_data.get('taxes', []):
            ...
            for section in tax_type.get('sections', []):
                ...
                if not section.get('rates', []):
                    ws.append([
                        airport_code, country_name, code, name, terminals,
                        '—', '—', '—', '—'
                    ])
                    for col in range(1, len(headers) + 1):
        ...
        for tax_type in airport_data.get('taxes', []):
            ...
            for section in tax_type.get('sections', []):
                ...
                for rate in section.get('rates', []):
                    ...
                    ws.cell(row=row_idx, column=1, value=rate.get('condition', '')).border = THIN_BORDER
                    ws.cell(row=row_idx, column=2, value=rate.get('currency', '')).border = THIN_BORDER
                    
                    amt_cell = ws.cell(row=row_idx, column=3, value=rate.get('amount', ''))
